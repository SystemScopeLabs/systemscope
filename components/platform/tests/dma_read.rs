...
//! the WRITE boundary (M2.7b), protocol violations, snapshots of the reachable READ
...
// --- The WRITE boundary (M2.7b) ---

#[test]
fn a_write_command_stays_busy_and_sends_nothing() {
    let mut w = World::new();
    w.write_reg(R_IRQ_ENABLE, 1);
    let ram = w.ram.clone();
    w.command(WRITE, 2, 0x1000, 2);
    assert!(!w.wake && w.outstanding.is_none(), "no wake, no request");
    assert!(!w.step());
    assert_eq!(w.status(), BUSY);
    assert!(w.log.is_empty() && w.levels.is_empty());
    assert!(w.ram == ram);
    assert_eq!(w.traced, [command_record(2, 2, 0x1000, 2, true)]);
    let v = view(&w.c);
    assert_eq!(
        (v.latched, v.engine, v.block, v.beat, v.buffer.len()),
        (Some((2, 2, 0x1000, 2)), 1, 0, 0, 0)
    );
    assert_eq!((v.dma_txn, v.blk_txn), (0, 0));
    // It survives a checkpoint unchanged, and ACK cannot end it.
    w.checkpoint();
    w.write_reg(R_ACK, 1);
    assert_eq!(w.status(), BUSY);
    assert!(!w.step());
