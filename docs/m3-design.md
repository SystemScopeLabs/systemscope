...
- **Interim switch point.** Until M3.5, an `ecall` from U (scause 8) is the cooperative switch: the frame becomes the caller's context with `sepc + 4`, the caller goes to the queue's tail, and the head is dispatched. No register is decoded, and no guest-visible result is written. M3.5 replaces it with the §6.5 dispatch, routing `sched_yield` to the same transition. With no `exit` yet, no process ends with status 0, so the empty-queue shutdown reason is 1.
