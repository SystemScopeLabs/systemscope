//! M1-A7: the M0 observation configurations O0 to O5 on `m1-reference`
//! (`docs/m1-design.md` §10.1, `docs/m0-design.md` §9 AT-3).
//!
//! The configurations and the invariance checks are M0's own ([`Config`],
//! [`ensure_invariant`](crate::observation::ensure_invariant),
//! [`ensure_same_trace`](crate::observation::ensure_same_trace)). Only the two things
//! M1-A7 fixes differ: the breakpoint in O2 pauses on every CPU `Commit` event, and the
//! probe in O4 inspects every component of `m1-reference`.
...
use systemscope_rv32::runner::CPU;
...
impl Observer for Probe {
    fn on_observe(&mut self, _: Tick, world: &WorldView<'_>) -> Control {
        ...
        for id in 0..world.component_count() {
            let id = ComponentId(u32::try_from(id).expect("four components"));
...
/// Runs `program` under `config` to its end. O4 places observe points from tick 0 through
/// `last_tick`, the unobserved run's last event.
///
/// # Panics
///
/// If the run faults, which no committed program does.
pub fn observe(program: &Program, config: Config, last_tick: Tick) -> Observed {
    ...
    rt.init().expect("m1-reference initializes");
